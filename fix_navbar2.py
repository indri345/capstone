...
files = [
    ...
    'cms/templates/cms/explore.html',
    'cms/templates/cms/culture_performance.html',
    'cms/templates/cms/business_performance.html',
...
font_import = '<link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600&display=swap" rel="stylesheet">'
...
for f in files:
    ...
    content = re.sub(r'\.navbar a:hover\s*\{[^}]*\}', '', content)
    content = re.sub(r'\.active-link\s*\{[^}]*\}', '', content)
    content = re.sub(r'\.admin-btn\s*\{[^}]*\}', '', content)
    content = re.sub(r'\.brand-link\s*\{[^}]*\}', '', content)
    content = re.sub(r'\.brand-logo\s*\{[^}]*\}', '', content)
    content = re.sub(r'/\*\s*NAVBAR CSS\s*\*/', '', content)
...
print("Updated all navbars with Poppins font and container-fluid px-4 successfully!")
